...
const targetStr = `        window.populateCategoryDropdowns = function() {
...
const targetStrAlt = `        window.populateCategoryDropdowns = function() {
            const apCat = document.getElementById('apCategory');
            if(apCat) {
                apCat.innerHTML = '<option value="" disabled selected>پۆلێنێک هەڵبژێرە...</option>';
                allCategories.forEach(c => {
...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log('Successfully updated category dropdown logic!');
} else {
    console.log('Could not find populateCategoryDropdowns');
}
